...
def main():
    xmr_output_file = open('xmr_xmrv9_addr.dat', 'rb')
    ...
    cur.execute('SELECT * FROM xmr_xmrv9_keyimages;')
    ...
    for (image, id, ring_amount, ring_indices, distinct_ring_indices, block_height, fork_indices) in xmr_records:
        ...
        cur.execute('SELECT * FROM xmrv9_keyimages WHERE image=%s;', (image,))
        xmrv9_record = cur.fetchall()[0]
        other_ring_indices = xmrv9_record[-2]
        other_ring_amount = xmrv9_record[2]
        assert(other_ring_amount == ring_amount)
        xmrv9_ring_addresses = []
        for index in other_ring_indices:
            cur.execute('SELECT * FROM xmrv9_outputs WHERE amount=%s AND index=%s', (other_ring_amount, index))
            xmrv9_output = cur.fetchall()[0]
            xmrv9_ring_addresses.append(xmrv9_output[0])
        int_ring_addresses = list(set(xmr_ring_addresses) & set(xmrv9_ring_addresses))
        int_ring_indices = list(map(lambda a: xmr_out_amt_idx_dict[a][1], int_ring_addresses))
        
        cur.execute('UPDATE xmr_xmrv9_keyimages SET fork_indices=%s WHERE image=%s', (f'{{{", ".join(map(str, int_ring_indices))}}}', image))
    
    cur.execute('SELECT ARRAY_LENGTH(fork_indices,1) FROM xmr_xmrv9_keyimages;')
    result = cur.fetchall()
