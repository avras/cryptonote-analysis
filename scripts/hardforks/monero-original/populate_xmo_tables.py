...
def main():
    ...
    for i in range(min_height, max_height+1):
        getblock_payload["params"] = {"height":i}
        ...
        if tx_hashes != []:
            ...
            for tx in rt['txs']:
                ...
                for txi in txinputs:
                    ...
                    cur.execute('INSERT INTO xmo_keyimages (image, ring_amount, ring_indices, block_height) VALUES(%s, %s, %s, %s) ON CONFLICT(image) DO NOTHING;', (keyimage, amount, keyindices, blockheight))
                    for ki in keyindices:
                        ...
                        cur.execute('INSERT INTO xmo_outputs (address, amount, index) VALUES(%s, %s, %s) ON CONFLICT(amount, index) DO NOTHING;', (address, amount, ki))
            conn.commit()
