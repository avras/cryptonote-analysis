...
def main():
    ...
    max_height = 1711290
    #max_height = 1000
    url_getblock = 'http://127.0.0.1:19091/json_rpc'
    url_gettransactions = 'http://127.0.0.1:19091/gettransactions'
    url_getouts = 'http://127.0.0.1:19091/get_outs'
    getblock_payload = {
        "method": "getblock",
        "jsonrpc": "2.0",
        "id": "0"
...
#        print(blockheight, len(tx_hashes))
        if tx_hashes != []:
            ...
            for tx in rt['txs']:
                ...
                for txi in txinputs:
                    ...
                    cur.execute('INSERT INTO xmv_keyimages (image, ring_amount, ring_indices, block_height) VALUES(%s, %s, %s, %s) ON CONFLICT(image) DO NOTHING;', (keyimage, amount, keyindices, blockheight))
                    for ki in keyindices:
#                        cur.execute('INSERT INTO xmv_outputs (amount, index) VALUES(%s, %s) ON CONFLICT(amount, index) DO NOTHING;', (amount, ki))
                        ...
                        #print(address)
                        cur.execute('INSERT INTO xmv_outputs (address, amount, index) VALUES(%s, %s, %s) ON CONFLICT(amount, index) DO NOTHING;', (address, amount, ki))
            conn.commit()
